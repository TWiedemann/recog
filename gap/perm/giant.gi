...
## The permutation conj lies in SymmetricGroup(lp) where lp is the largest moved point
...
RECOG.ConjEltSn :=  function( grp, fullCycle, transp )
    ...
    local c,i,lp,n,rest,pos1,nextPos,suppTransp,mp;

    mp := MovedPoints(grp); # = MovedPoints(fullCycle)
    n := Length(mp);
    lp := mp[n];   # this is the largest moved point
    ...
    od;
    # The remaining points in [1..lp] must be mapped bijectivey to [n+1..lp] to ensure that
    # conj is a permutation. However, the precise values of individual points are irrelevant.
    rest := Difference([1..lp],mp);
...
RECOG.ConjEltAnEven := function( grp, longPerm, cyc3 )
    local c,i,lp,n,rest,pos,s1,s1lo,supp3,mp,p1,p2,p3;
...
RECOG.ConjEltAnOdd := function( grp, longCycle, shortCycle )
    local c,compt,i,lp,n,rest,pos,mp;
